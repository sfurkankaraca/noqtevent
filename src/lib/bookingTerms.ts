...
// Ön ödemeli plan vade farkı oranı artık booking.prepay_markup_rate ile
// admin tarafından ayarlanır (bookings tablosu, varsayılan %25) — bu sabit
...
// markupRatePercent: booking.prepay_markup_rate (örn. 25 → %25 vade farkı)
export function calcPrepayPrice(fee: number, markupRatePercent?: number | null): number {
  const rate = markupRatePercent ?? DEFAULT_PREPAY_MARKUP_RATE;
  return Math.round(fee * (1 + rate / 100));
}
...
1. Fiyatlandırma: Peşin ödeme seçeneğinde belirtilen ücret geçerlidir. Ön ödemeli (kaporalı) rezervasyon seçeneğinde toplam ücrete teklifte belirtilen oranda vade farkı uygulanır.
