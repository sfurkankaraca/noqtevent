              ...
              <div className="p-8 space-y-6">
                  ...
                  <div>
                    ...
                    <div className="space-y-3">
                      {PERFORMER_TYPES.map((pt) => (
                        <button key={pt.id} type="button" onClick={() => setPerformerType(pt.id)}
                          className={`w-full flex items-center gap-4 p-4 rounded-xl border text-left transition-all ${performerType === pt.id ? "border-foreground bg-foreground/5" : "border-border hover:border-foreground/40"}`}>
                          ...
                          {performerType === pt.id && <span className="ml-auto text-foreground">✓</span>}
                        </button>
                      ))}
                    </div>
                  </div>
                ...
                {step === 1 && (
                  <div className="space-y-5">
                    ...
                    <div>
                      <label className="block text-xs font-medium text-foreground mb-1.5">Sahne Adı / Ekip Adı <span className="text-red-500">*</span></label>
                      <input className={inputCls} value={name} onChange={(e) => setName(e.target.value)} placeholder="DJ Nova, Ece Güler, The Jazz Trio…" />
                    </div>
                    <div>
                      <label className="block text-xs font-medium text-foreground mb-1.5">Uzmanlık / Müzik Stili</label>
                      <input className={inputCls} value={speciality} onChange={(e) => setSpeciality(e.target.value)} placeholder="Deep House, Latin Dans, Caz, Türkçe Pop…" />
                    </div>
                    <div>
                      <label className="block text-xs font-medium text-foreground mb-1.5">Hakkında <span className="text-red-500">*</span></label>
                      <textarea className={inputCls + " resize-none"} rows={4} value={bio} onChange={(e) => setBio(e.target.value)} placeholder="Kendinizi tanıtın. Deneyimleriniz, tarzınız, neyi farklı kıldığınız…" />
                      <p className="text-xs text-muted-foreground mt-1">{bio.length} karakter</p>
                    </div>
                    <div>
                      <label className="block text-xs font-medium text-foreground mb-1.5">Repertuar</label>
                      <textarea className={inputCls + " resize-none"} rows={3} value={repertoire} onChange={(e) => setRepertoire(e.target.value)} placeholder="Hangi dillerde / türlerde çalıyorsunuz? ör. Türkçe pop, Arapça, Flamenko, 90s hits…" />
                    </div>
                    <div>
                      <label className="block text-xs font-medium text-foreground mb-3">Hizmet Verdiğiniz Şehirler</label>
                      <div className="flex flex-wrap gap-2 mb-3">
                        {CITIES.map((city) => (
                          <button key={city} type="button" onClick={() => toggleCity(city)}
                            className={`px-3 py-1.5 rounded-full text-xs font-medium border transition-all ${coverCities.includes(city) ? "bg-foreground text-background border-foreground" : "border-border text-foreground hover:border-foreground/40"}`}>
                            {city}
                          </button>
                        ))}
                      </div>
                    ...
                    <div className="grid grid-cols-2 gap-3">
                      {EVENT_TYPES.map((et) => (
                        <button key={et.id} type="button" onClick={() => toggleEventType(et.id)}
                          className={`flex items-center gap-3 p-3.5 rounded-xl border text-left transition-all ${selectedEventTypes.includes(et.id) ? "border-foreground bg-foreground/5" : "border-border hover:border-foreground/40"}`}>
                            ...
                            <div key={i} className="relative aspect-square rounded-xl overflow-hidden border border-border bg-secondary/20">
                              {ph.uploading ? (
                                ...
                                <>
                                  ...
                                  {i === 0 && <span className="absolute top-1.5 left-1.5 bg-black/60 text-white text-[9px] px-1.5 py-0.5 rounded-full">Kapak</span>}
                ...
                {step === 4 && (
                  <div className="space-y-5">
                    <div>
                      ...
                      <p className="text-sm text-muted-foreground">Başvurunu değerlendirmek için kullanacağız.</p>
                    </div>
                    <div>
                    ...
                    <div className="bg-[oklch(0.975_0.006_80)] rounded-xl p-4 space-y-2">
                      ...
                      {[
                        ["Tür", `${PERFORMER_TYPES.find((p) => p.id === performerType)?.emoji} ${PERFORMER_TYPES.find((p) => p.id === performerType)?.label}`],
                        ["Sahne Adı", name],
                        ["Fotoğraf", `${photos.filter((p) => p.url && !p.error).length} adet`],
                        ["Etkinlik Tipleri", `${selectedEventTypes.length} seçim`],
                        ["Video", `${youtubeLinks.filter(Boolean).length} link`],
                      ].map(([k, v]) => (
                ...
                <div className="flex items-center justify-between pt-2">
                  ...
                  {step < STEPS.length - 1 ? (
                    ...
                    <button type="button" onClick={handleSubmit} disabled={loading || !canProceed() || photoUploading}
