...
export default function PlannerWizard({ conceptCovers = {}, activeSlugs, djs = [], venues = [] }: { conceptCovers?: Record<string, string>; activeSlugs?: string[]; djs?: Dj[]; venues?: Venue[] }) {
  ...
  const openPdf = () => {
    ...
    window.open(`/planla/ozet?d=${encodeURIComponent(encoded)}`, "_blank");
  };
    ...
    return (
      <div className="min-h-screen flex items-center justify-center px-6 bg-[oklch(0.975_0.006_80)]">
        ...
        >
          ...
          {/* PDF Download */}
          <div className="mt-8 p-5 bg-white rounded-2xl border border-border text-left space-y-3">
            <div className="flex items-center gap-3">
              ...
              <div>
                <p className="text-sm font-semibold text-foreground">Etkinlik Deneyim Taslağı</p>
                <p className="text-xs text-muted-foreground">Müzik akışı, konseptler ve hizmet talepleri</p>
              </div>
            </div>
            <p className="text-xs text-muted-foreground leading-relaxed">
              Taslağını PDF olarak indir. Düğün salonu veya organizatörünle paylaşabilir, referans olarak kullanabilirsin.
            </p>
            <button
              onClick={openPdf}
              className="w-full flex items-center justify-center gap-2 bg-foreground text-background py-3 rounded-xl text-sm font-medium hover:opacity-90 transition-opacity"
            >
